# Cardinal Nest Monitor — Project Memory for Codex
...
- Every snap → `Codex-sonnet-4-6` directly (no Haiku prefilter). Single model call per snap.
...
The full design rationale is in `/Users/jasbali/.Codex/plans/reactive-tickling-rose.md`.
...
ANALYZER SERVICE (com.cardinalnest.analyzer):
  Polls data/spool/pending/ every 1s → claims newest snap first
    → analyzer.analyze() via Codex-sonnet-4-6 (~$0.01)
...
| Setting | Value | Why |
|---|---|---|
| `ANALYZER_MODEL` | `Codex-sonnet-4-6` | Single-tier. Was Opus 4.6. Sonnet is ~5x cheaper with no meaningful accuracy regression for threat detection. |
| `PREFILTER_MODEL` | `Codex-haiku-4-5-20251001` | **Unused in single-tier mode.** Kept in config for easy re-enable. |
...
launchd/com.cardinalnest.monitor.plist       Legacy combined LaunchAgent (rollback). Installed at ~/Library/LaunchAgents/.
...
- **`!` prefix in Codex** runs commands without a TTY → `input()` raises `EOFError` immediately.
...
The user's first API key (`...pSeXRQAA`) lived in a workspace with $0 credits even though their billing page showed $20. Anthropic accounts can have multiple workspaces; **each workspace has its own credit pool** and API keys are workspace-scoped. The fix was creating a fresh key in the funded workspace (current key ends `...ipzlWwAA`).
...
Several times during development, plan mode got reactivated mid-task (Shift+Tab toggle), preventing edits. The Agent tool also inherits plan-mode permissions when spawned, so parallel implementation agents got stuck writing sub-plans they couldn't execute. Workaround: read their generated plan files (in `~/.Codex/plans/`) and apply the writes from the main session.
...
**Don't revert this** without a careful analysis. A future Codex might be tempted to "optimize" by restoring fire-and-forget for parallelism — the cost is that Pattern A's absence-aware cadence becomes off-by-one-iteration again, which is a life-critical defect for the eggs.
...
**Don't regress this.** The prompt section headed "== Species identification — READ CAREFULLY ==" is load-bearing. If a future Codex tries to shorten or "clean up" the prompt, they'll re-introduce the cardinal/thrasher confusion. Validate any prompt change via the regression test suite (next section).
...
analyzer.analyze(jpeg, model_override="Codex-opus-4-6", extra_user_text=<nudge>)
    ↓
...
**Alert embed format:** when verification runs, the embed gets an extra `✓ Verification (Codex-opus-4-6)` field showing Opus's confidence and summary. If Opus DISAGREED (suppressed), the alert doesn't fire at all — only a log line captures the suppression. Look for `Opus verification: ... → SUPPRESSED` in `~/Library/Logs/cardinal-nest-monitor/out.log` to see what Opus caught.
...
```bash
cd /Users/jasbali/repos/Codex/Codex/repos/cardinal-nest-monitor
source venv/bin/activate
...
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.cardinalnest.monitor.plist
```
...
**Don't regress this.** A future Codex might be tempted to merge the two services back into one "for simplicity." That is the exact path that produced the 2026-04-15 outage cascade. The decoupled architecture exists so that the volatile analyzer code (Anthropic calls, Discord webhooks, rules engine, verifier) can be iterated on without ever interrupting the stable downloader (Blink API + disk writes). If you need to change both, restart both — but the default deploy path should only touch the analyzer. The downloader changes maybe once a quarter; the analyzer changes weekly.
...
`DISCORD_FEED_WEBHOOK_URL` (in `.env`) enables a parallel "every-snap" feed channel — receives one embed per snap with the JPEG attached and Codex's text reply. Designed for sharing visibility / watching the system reason in real time.
...
7. **Swap analyzer model** from `Codex-sonnet-4-6` to `Codex-haiku-4-5` — cheapest but unreliable on IR (see §8). Not recommended.
