...
def _default_species_profile_path() -> Path:
    ...
    the cycle.
    """
    # Deferred import: birdnest_ai.species also imports from
    # this module (config.get_settings). A top-level import would cycle.
    import importlib.resources as _res

    ref = _res.files("birdnest_ai.species.profiles") / "northern_cardinal.toml"
    return Path(str(ref))
...
class Settings(BaseSettings):
    ...
    # Regression: `python -m birdnest_ai.tools.lifecycle_regression`
    ...
    # startup via birdnest_ai.species.get_species_profile() and
