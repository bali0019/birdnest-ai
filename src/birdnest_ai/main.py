...
Usage:
    python -m birdnest_ai              # run the full system
    python -m birdnest_ai --auth-only  # one-time interactive 2FA setup
"""
...
from birdnest_ai.cadence import (
...
from birdnest_ai.species.loader import bootstrap_species_profile
...
class Pipeline:
    ...
    async def on_clip(self, cam, clip: dict[str, Any]) -> None:
        ...
        try:
            ...
            from birdnest_ai.blink_client import (  # local import for binding
                download_clip as _dl,
            )
            from birdnest_ai.blink_client import connect as _connect  # noqa
...
async def run_downloader() -> int:
    ...
    Thin delegate to `birdnest_ai.downloader_loop.run_downloader_service`.
...
async def run_analyzer() -> int:
    ...
    Thin delegate to `birdnest_ai.analyzer_loop.run_analyzer_service`.
    """
    from birdnest_ai.analyzer_loop import run_analyzer_service
