"""Discord webhook notifier for Birdnest AI.
...
from birdnest_ai.schema import AlertDecision, NestObservation, PrefilterResult, Severity
...
def _footer() -> dict[str, str]:
    ...
    base = f"Birdnest AI • {datetime.now().strftime('%Y-%m-%d %H:%M')}"
...
class Notifier:
    ...
    async def send_test(self) -> bool:
        embed = {
            "title": "🧪 Webhook test",
            "description": "Birdnest AI webhook is wired up correctly.",
            "color": 0x00FF00,
            "timestamp": _now_iso_utc(),
            "fields": [
                {"name": "Camera", "value": self.camera_name or "—", "inline": True},
    ...
    async def send_analytics_report(self, report: dict[str, Any]) -> bool:
        ...
        embed: dict[str, Any] = {
            ...
            "fields": fields,
            "footer": {
                "text": f"Birdnest AI • analytics • {window_hours}h window",
            },
        }
        return await self._send_json({"embeds": [embed]})
