...
via :func:`birdnest_ai.spool.write_snap`.
...
async def run_downloader_service() -> int:
    ...
    try:
        await asyncio.wait_for(
            notifier.send_system_message(
                title="🟢 Birdnest AI downloader online",
                body=(
    ...
    try:
        ...
        try:
            ok = await asyncio.wait_for(
                notifier.send_system_message(
                    title="🔴 Birdnest AI downloader offline",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="birdnest_ai.downloader_loop",
        description=(
