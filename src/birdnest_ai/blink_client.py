...
# vector that existed when the file lived at /tmp/birdnest_ai_blink_pin.
PIN_FILE_PATH = Path.home() / ".cache" / "birdnest_ai" / "blink_pin"
...
async def _read_2fa_pin() -> str:
    """Get a 2FA PIN from one of three sources, in order:
      ...
      3. File at $HOME/.cache/birdnest_ai/blink_pin (polled for up to 5 minutes)
    ...
    pick up the corrected version. To drop the PIN:
        ...
        install -m 600 /dev/stdin ~/.cache/birdnest_ai/blink_pin <<< "123456"

    or equivalently:

        umask 077
        printf '%s' "123456" > ~/.cache/birdnest_ai/blink_pin
